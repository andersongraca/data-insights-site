...
import { ChevronRight, BarChart3, Zap, Database, TrendingUp, Lock, Brain } from 'lucide-react';
...
interface ExpandedSolutionsMenuProps {
  onSelectSolution?: (solutionId: string) => void;
}

const ExpandedSolutionsMenu = ({ onSelectSolution }: ExpandedSolutionsMenuProps) => {
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {solutions.map((solution) => (
            <div
              ...
              className="group relative bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-6 border border-gray-200 hover:border-green-300 transition-all cursor-pointer hover:shadow-lg hover:-translate-y-1"
            >
              {/* Icon Background */}
              <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${solution.color} opacity-5 rounded-full -mr-8 -mt-8 group-hover:opacity-10 transition-opacity`}></div>

              {/* Content */}
              <div className="relative z-10">
                {/* Icon */}
                <div className={`inline-flex p-3 rounded-lg bg-gradient-to-br ${solution.color} text-white mb-4 group-hover:scale-110 transition-transform`}>
                  {solution.icon}
                </div>
...
export default ExpandedSolutionsMenu;
